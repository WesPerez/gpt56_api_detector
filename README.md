...
本 fork 的 `main` 跟随上游，`mine` 分别保留部署规范和日志维护两项定制。
本服务器禁止安装或下载项目依赖、编译、构建及打包；源码审查后推送 GitHub，
由 GitHub-hosted Actions 测试并生成发布包。部署只使用核对完整 Git SHA 和
校验和的 CI 产物，沿用现有运行环境、数据目录和端口，不运行源码包安装器。

普通 `events` 诊断日志保留 3 天、最多 100,000 条，按最旧记录分批清理。
活跃任务暂时豁免；维护不阻塞请求循环，并在关闭数据库前等待在途清理完成。
完整检测报告及其冻结事件副本、请求交换留存、会话、预算和凭证属于业务产物，
不按普通日志清除。其他服务器维护边界见 [AGENTS.md](AGENTS.md)。
